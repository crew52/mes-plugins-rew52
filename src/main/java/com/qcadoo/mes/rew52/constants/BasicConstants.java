package com.qcadoo.mes.rew52.constants;

public class BasicConstants {
    private BasicConstants() {

    }

    public static final String PLUGIN_IDENTIFIER = "rew52";

    // MODEL
    public static final String MODEL_EMPLOYEE = "employee";
    public static final String MODEL_DEPARTMENT = "department";
    public static final String MODEL_POSITION = "position";
    public static final String MODEL_CONTRACT = "contract";
    public static final String MODEL_WORKING_SHIFT = "workingShift";
    public static final String MODEL_EMPLOYEE_SHIFT_ASSIGNMENT = "employeeShiftAssignment";
    public static final String MODEL_ABSENCE = "absence";

}
